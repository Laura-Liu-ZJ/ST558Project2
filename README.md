...
In this project, I try to use a logistic regression model (linear model)
and a bagged tree model (non-linear model) to predict the number of
shares in social networks.

# Data

The full dataset have 39644 observations and 61 variables. However,
there are only 1 response and 23 predictors we interested. And following
is the description of variables we cared about.
...
In addition, we divid the `shares` into two groups (\< 1400 and \(\ge\)
...
From above 23 potential predictors, we use mallow’s cp and BIC to choose
the variables which can be used in the linear model. Therefore, we use
`newsDataFit` data set for fitting linear model and use `newsDataTrain`
data set for fitting non-linear model.
...
exact number of each share group. Furthermore, we should standardize our
variables before fitting model due to the extreme differences on maximum
of each variables.
...
According to the contingency table, we caculate the accuracy for each
data set by treating all the shares as in the “\(\ge\) 1400” group.
...
We use the variables choose from mallow’s cp and BIC as our predictors
in the linear model. Therefore, we use `newsDataFit` data set for
fitting linear model. But before the fitting the model we should
standardize our data first. And then, we need to caculate odds for given
predictors, and convert them into binary variables. After prediction
process, we using confusion matrix to get the accuracy of linear model
...
We use `newsDataTrain` data set for fitting bagged tree model. The
bagged tree model is a method combining bootstrap and tree model
together. It create a bootstrap sample first and then train tree on this
sample. Also, we standardize the variables before fitting the model.
...
then, we need to caculate odds for given predictors, and convert them
into binary variables. After prediction process, we use confusion matrix
to get the accuracy of linear model for testing data set.
...
We use our bagged tree model to predict the sharesInd in the testing
data set. And then, we caculate the accuracy of testing data set by
confusion matrix.
...
According to the results of APER table, we would choose the smallest
value as the best model for each data set . In this case, our best model
for training data set is *ne* and the best model for testing data set is
*ne*. What’s more, if the best model turns out to be the Original Data,
neither linear model nor non-linear model had done the job well.
...
|                  |  Train |   Test |
| ---------------- | -----: | -----: |
| Linear Model     | 0.6336 | 0.6283 |
| Non-linear Model | 0.6137 | 0.6358 |
...
|                  |  Train |   Test |
| ---------------- | -----: | -----: |
| Linear Model     | 0.3664 | 0.3717 |
| Non-linear Model | 0.3863 | 0.3642 |
